import type { KnowledgeNode, KnowledgeLink } from './KnowledgeIndex';
// 带坐标的图节点
...
export class GraphLayout {
    ...
    init(knowledgeNodes: KnowledgeNode[], links: KnowledgeLink[], maxNodes: number = 100): void {
        ...
        const filteredLinks = links.filter((link: KnowledgeLink) => nodeSet.has(link.source) && nodeSet.has(link.target) && link.strength >= 0.2);
