...
import * as writers from "@package:pkg_modules/.ohpm/@ohos+mammoth@1.0.0/pkg_modules/@ohos/mammoth/src/main/ets/writers/index";

export function DocumentConverter(options) {
    return {
...
function DocumentConversion(options, comments) {
    ...
    async function convertToHtml(document) {
        ...
        const mapper = (deferred) => {
            return deferred.value().then(function(value) {
                deferredValues[deferred.id] = value;
            });
        };

        for( let i = 0;i < deferredNodes.length; i++ ){
            await mapper(deferredNodes[i]);
        }
