...
import options from "@package:pkg_modules/.ohpm/@ohos+mammoth@1.0.0/pkg_modules/@ohos/mammoth/src/main/ets/lop/option/index";
import results from "@package:pkg_modules/.ohpm/@ohos+mammoth@1.0.0/pkg_modules/@ohos/mammoth/src/main/ets/lop/lib/parsing-results";
import * as errors from "@package:pkg_modules/.ohpm/@ohos+mammoth@1.0.0/pkg_modules/@ohos/mammoth/src/main/ets/lop/lib/errors";
import * as lazyIterators from "@package:pkg_modules/.ohpm/@ohos+mammoth@1.0.0/pkg_modules/@ohos/mammoth/src/main/ets/lop/lib/lazy-iterators";

export const token = function(tokenType, value) {
...
export const tokenOfType = function(tokenType) {
    return token(tokenType);
};

export const firstOf = function(name, parsers) {
...
export const then = function(parser, func) {
    return function(input) {
...
export const sequence = function() {
    ...
    var rule = function(input) {
        ...
        return result.map(function(values) {
            return values.withValue(sequence.source, source);
        });
    };
    rule.head = function() {
...
sequence.capture = function(rule, name) {
...
sequence.extract = function(rule) {
    return function(result) {
        return result.get(rule);
    };
};

sequence.applyValues = function(func) {
...
sequence.cut = function() {
    return function(input) {
        return results.cut(input);
    };
};

export const optional = function(rule) {
    return function(input) {
...
export const zeroOrMoreWithSeparator = function(rule, separator) {
    return repeatedWithSeparator(rule, separator, false);
};

export const oneOrMoreWithSeparator = function(rule, separator) {
    return repeatedWithSeparator(rule, separator, true);
};

export var zeroOrMore = function(rule) {
    return function(input) {
        var values = [];
        var result;
...
export const oneOrMore = function(rule) {
    return oneOrMoreWithSeparator(rule, noOpRule);
};
...
var repeatedWithSeparator = function(rule, separator, isOneOrMore) {
    return function(input) {
        var result = rule(input);
        if (result.isSuccess()) {
            var mainRule = sequence.capture(rule, "main");
            var remainingRule = zeroOrMore(then(
                sequence(separator, mainRule),
                sequence.extract(mainRule)
            ));
...
export const leftAssociative = function(leftRule, rightRule, func) {
    var rights;
    if (func) {
...
export const nonConsuming = function(rule) {
    return function(input) {
